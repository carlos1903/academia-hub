...
@section('title', 'Editar Curso - Academia Hub')
        ...
        <form action="{{ route('cursos.update', $curso) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="codigo" id="codigo" value="{{ old('codigo', $curso->codigo) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent" required>
                    @error('codigo')
                ...
                <div>
                    ...
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $curso->nombre) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent" required>
                    @error('nombre')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="nivel" id="nivel" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent" required>
                        ...
                        <option value="Bachillerato" {{ old('nivel', $curso->nivel) == 'Bachillerato' ? 'selected' : '' }}>Bachillerato</option>
                    </select>
                    @error('nivel')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex items-center gap-4 pt-4 border-t border-gray-100 mt-6">
                <button type="submit" class="bg-yellow-400 hover:bg-yellow-500 text-white font-semibold py-3 px-6 rounded-lg transition flex items-center gap-2">
                    <i class="fas fa-sync-alt"></i> ACTUALIZAR CURSO
                </button>
